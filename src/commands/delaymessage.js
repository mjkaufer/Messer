  ...
  return {
    ...
    handler(command) {
      return new Promise((resolve, reject) => {
        ...
        if (
          rawDelay.length === 0 /*|| !Number.isInteger(minDelay) */ ||
          !(rawDelay >= 1 && rawDelay <= 1440)
        ) {
          return reject(
            Error("Invalid delay - it has to be an integer between 1-1440"),
          );
        } else {
          ...
          console.log(
            "Waiting ",
            rawDelay,
            " minutes to send the message" /* to ',thread.name*/,
          );
            ...
            .then(thread => {
              ...
              return messer.messen.api.sendMessage(
                ...
                err => {
                  ...
                  return resolve(
                    `Sent message to ${thread.name} with ${rawDelay} minutes delay`,
                  );
                },
              );
            })
